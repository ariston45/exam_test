...
function connectdb()
{
	...
	$GLOBALS['link'] = mysqli_connect($host, $username, $password, $db_name,'3307');
...
function cekAdminLogin()
{
	if (isset($_SESSION['admin_level'])) {
		if (($_SESSION['admin_level'] == 'Administrator') or ($_SESSION['admin_level'] == 'PIC') or ($_SESSION['admin_level'] == 'Exam Administrator') or ($_SESSION['admin_level'] == 'Student Register') or ($_SESSION['admin_level'] == 'Proctor') or ($_SESSION['admin_level'] == 'Program Manager') or ($_SESSION['admin_level'] == 'Admin Office') or ($_SESSION['admin_level'] == 'Marketing Manager') or ($_SESSION['admin_level'] == 'Support')
		) {
...
function CovidcekTimeSchedule($id_schedule)
{
	$sql = "SELECT start_time,date from covid_exam_schedule
	WHERE id_schedule = $id_schedule";
	//die($sql);
	$rs = mysqli_query($GLOBALS['link'], $sql);
	$today = timeVar();
	$row = mysqli_fetch_row($rs);
	$start_time7 = strtotime("07:00:00");
	$end_time7 = strtotime("19:00:00");
	$startTime = date('H:i:s', $start_time7);
	$endTime = date('H:i:s', $end_time7);
	//07:00 sampai 19:00
	if ($today[0] == $row[1]) {
		if ($today[1] > $startTime) {
			if ($today[1] < $endTime) {
				$list[0] =  1;
			} else {
				$list[0] =  0;
...
function CovidcekLogStu($user_id, $password)
{
	$sql = "SELECT a.id_schedule,a.exam_group,a.token, b.count_stu
	FROM covid_exam_schedule a INNER JOIN exam_group b ON a.exam_group = b.exam_code
	WHERE a.`status`='run' AND a.token='$password'";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
	$dat = mysqli_fetch_array($res);
	$idg = $dat[1];
...
{
	$res = mysqli_query($GLOBALS['link'], 'SELECT a.* FROM customer a ORDER BY a.cust_name ASC');
	return $res;
}
// ================================================================================================================
// PIC voucher
...
function vouPre()
{
	...
	WHERE a.id_customer='$id' AND a.type_voucher ='Prepaid' AND b.available_val > 0";
	$res = mysqli_query($GLOBALS['link'], $sql);
...
function updateUser($id, $name, $dob, $pob, $address, $phone, $email, $role, $uname)
{
...
function showStudents_var_ii($id_g, $valin)
{
	$id = $_SESSION['admin_group'];
	$sql = "SELECT a.* FROM students a WHERE SUBSTRING_INDEX(a.our_id,'.',1)='$id' AND a.idstudents LIKE '%$valin%' OR a.fname LIKE '%$valin%'";
	$res = mysqli_query($GLOBALS['link'], $sql) or die('errn');
	$dat = array();
	while ($row = mysqli_fetch_array($res)) {
		$dat[] = $row;
...
{
	$id = $_SESSION['admin_group'];
	$sql = "SELECT a.* FROM students a WHERE SUBSTRING_INDEX(a.our_id,'.',1)='$id'";
	$res = mysqli_query($GLOBALS['link'], $sql) or die('errn');
	$dat = array();
	while ($row = mysqli_fetch_array($res)) {
		$dat[] = $row;
...
function showStudentsAll($id_g)
{
	$sql = "SELECT a.* FROM (SELECT aa.idstudents,aa.fname,aa.email
	FROM students aa WHERE SUBSTRING_INDEX(aa.our_id,'.',1)='$id_g') a";
	$res = mysqli_query($GLOBALS['link'], $sql) or die('errn');
	$dat = array();
	while ($row = mysqli_fetch_array($res)) {
		$dat[] = $row;
...
function showStudentUnexap($id_g)
{
	$id = $_SESSION['admin_group'];
	$sql = "SELECT a.* FROM (SELECT aa.idstudents,aa.fname,aa.email FROM students aa
	WHERE SUBSTRING_INDEX(aa.our_id,'.',1)='$id') a LEFT JOIN
	(SELECT SUBSTRING_INDEX(bb.id_student,'.',1) id FROM exam_session bb GROUP BY bb.id_student) b
	ON a.idstudents = b.id WHERE b.id IS NULL";
	$res = mysqli_query($GLOBALS['link'], $sql) or die('errn');
	$dat = array();
	while ($row = mysqli_fetch_array($res)) {
		$dat[] = $row;
...
function showStudentUnexap_up($id_g)
{
	$id = $_SESSION['admin_group'];
	$sql = "SELECT s.idstudents,s.fname,s.email FROM db_exam.students s LEFT JOIN db_exam.exam_participants ep ON s.idstudents = SUBSTRING_INDEX(ep.id_student,'.',1) 
	WHERE s.our_id LIKE '$id%' AND ep.id_student IS NULL GROUP BY s.idstudents";
	$res = mysqli_query($GLOBALS['link'], $sql) or die('errn');
	$dat = array();
	while ($row = mysqli_fetch_array($res)) {
		$dat[] = $row;
...
function showStudentUnexapAdmin($id)
{
	$sql = "SELECT a.* FROM (SELECT aa.idstudents,aa.fname,aa.email FROM students aa
	WHERE SUBSTRING_INDEX(aa.our_id,'.',1)='$id') a LEFT JOIN
	(SELECT SUBSTRING_INDEX(bb.id_student,'.',1) id FROM exam_session bb GROUP BY bb.id_student) b
...
{
	$sql = "SELECT a.idstudents,a.fname,a.email,a.our_id FROM students a WHERE a.idstudents = '$ids'";
	$res = mysqli_query($GLOBALS['link'], $sql) or die('errn');
	$row = mysqli_fetch_array($res);
...
function updateDataStudent($ids, $nama, $email)
{
	$sql = "UPDATE students SET fname = '" . sqlValue($nama) . "', email ='$email'  WHERE idstudents = '$ids'";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
	return $res;
}

function updateDataStudentAdmin($ids, $nama, $email,$our_id)
{
	$sql = "UPDATE students SET fname = '" . sqlValue($nama) . "', email ='$email',idstudents='$ids'  WHERE idstudents = '$ids' AND our_id='$our_id'";
	$res = mysqli_query($GLOBALS['link'], $sql);
	return $res;
}
...
{
	$sql = "SELECT a.id_student,c.program_name,d.date,d.start_time,b.exam_code,c.id_program,e.notif,e.exam_code 
...
function CovidCekSchedule($exam_group)
{
	$sql = "SELECT d.date,d.start_time
	FROM  covid_exam_schedule d 
	WHERE d.exam_group = '$exam_group'";
	$res = mysqli_query($GLOBALS['link'], $sql) or die('errn');
	$dat = array();
	$row = mysqli_fetch_array($res);
...
{
	$sql = "SELECT a.id, b.subject_group, sum(a.percentage_true), sum(a.percentage_false), sum(a.percentage_null),b.id_subject FROM exam_percentage a LEFT JOIN subject_ls b ON a.id_subject=b.id_subject WHERE SUBSTRING_INDEX(a.id_student,'.',1)= '$id' AND a.exam_code='$idg' group by b.subject_group";
	$res = mysqli_query($GLOBALS['link'], $sql) or die('errn');
	$dat = array();
	while ($row = mysqli_fetch_array($res)) {
		$dat[] = $row;
...
function fileSetup()
{
	...
	$fileExtension = pathinfo($file1, PATHINFO_EXTENSION);
	if ($fileExtension != 'xls') {
		header("location:dashboard.php?pg=pic_add_student");
		echo "<script>alert('File Unsupported.');</script>";
...
function import_excel_studentsAdmin($filename, $id_g)
{
	$filename_path = '../' . $GLOBALS['tmp-dir'] . $filename;
	$ext = strtolower(array_pop(explode('.', $filename)));
	if ($ext == 'xls') {
	...
	$sql = "INSERT INTO students_reject_tmp SELECT a.* FROM students_up_tmp a INNER JOIN students b ON a.idstudent=b.idstudents
		WHERE SUBSTRING_INDEX(b.our_id,'.',1) ='{$id_g}'";
	mysqli_query($GLOBALS['link'], $sql); // insert ke students_reject_tmp jika data siswa sudah ada di table students
...
function saveStudents($filename)
{
	...
	$sql = "SELECT a.idstudent,a.fname,a.email,a.file_tmp FROM students_up_tmp a WHERE a.file_tmp='$filename'";
	$res = mysqli_query($GLOBALS['link'], $sql);
	$x = 0;
	while ($row = mysqli_fetch_array($res)) {
		$testTitik = strpos($row[0], ".");
		$idc = '';
		if ($testTitik != null) {
			$a = explode(".", $row[0]);
			$na = count($a);
			...
			$sqldeleteReject = "UPDATE students_up_tmp SET idstudent = $idc WHERE idstudent = " . $row[0];
			mysqli_query($GLOBALS['link'], $sqldeleteReject);
			$row[0] = $idc;
		}
		...
		$sql_in = "INSERT INTO students(idstudents, fname, email, batch_id, our_id, date) VALUES('$row[0]','" . sqlValue($row[1]) . "','$row[2]','$id_b $sqe_b', '$id_o $sqe_b', '$datetime')";
		$r = mysqli_query($GLOBALS['link'], $sql_in);
		if ($r == true) {
			++$sqe_b;
			++$sqe_o;
		} else {
			$sqlreject = "INSERT INTO students_reject_tmp VALUES ('" . $row[3] . "','789456','" . $row[0] . "','" . sqlValue($row[1]) . "','" . $row[2] . "')";
			mysqli_query($GLOBALS['link'], $sqlreject) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sqlreject);
			$sqldeleteReject = "DELETE FROM students_up_tmp WHERE idstudent = '" . sqlValue($row[0]) . "'";
			mysqli_query($GLOBALS['link'], $sqldeleteReject);
		}
	}
	mysqli_close($GLOBALS['link']);
}
function saveStudentsAdmin($filename, $id_g)
{
	$date = date('dmy');
	$time = date('H');
	$datetime = date('Y-m-d H:m:i');
...
function savePointStudent($id, $name, $email)
{
	...
	$sql_cekStudent = "SELECT idstudents FROM students WHERE idstudents = '$id' AND SUBSTRING_INDEX(our_id,'.',1) ='" . $_SESSION['admin_group'] . "'";
	$res_cekStudent = mysqli_query($GLOBALS['link'], $sql_cekStudent) or die(mysqli_error($GLOBALS['link']));
	if (mysqli_num_rows($res_cekStudent) == 0) {
		$sql = "INSERT INTO students(idstudents, fname, email, batch_id, our_id, date) VALUES('$id','" . sqlValue($name) . "','$email','$id_b','$id_o','$datetime')";
		$res = mysqli_query($GLOBALS['link'], $sql);
		if ($res === true) {
			return $id;
		} else {
...
function savePointStudentAdmin($id, $name, $email, $id_g)
{
	$date = date('dmy');
	$time = date('H');
	$datetime = date('Y-m-d h:m:i');
...
{
	$res = mysqli_query($GLOBALS['link'], "SELECT a.idstudent,a.fname,a.email,a.no FROM students_reject_tmp a where file_tmp = '$session_file' ") or die('err');
...
function resStudentRej($session_file)
{
	...
	if ($row = mysqli_num_rows($res) != 0) {
		...
		$noRj = 1;
		$tbRj = '<br><h4 style="color:D60000;">This ID is already used Please use another ID ex: 0000A or 0000B</h4>';
		$tbRj .=
			'<table ><thead><tr>
		<th scope = "col" > No </th>
		<th scope = "col" > Student ID </th>
		...
		while ($row = mysqli_fetch_array($res)) {
			if ($row[3] == 789456) {
				$tbRj .= '<tr><td>' . $noRj . '</td><td>' . $row[0] . '</td><td>' . $row[1] . '</td><td>' . $row[2] . '</td></tr>';
				++$noRj;
			} else {
				$table .= '<tr><td>' . $no . '</td><td>' . $row[0] . '</td><td>' . $row[1] . '</td><td>' . $row[2] . '</td></tr>';
				++$no;
...
function updateProgram($id, $name, $subject, $margin, $percent, $no_id, $tot, $duration)
{
	...
	foreach ($subject as $key => $value) {
		if ($value == null || $percent[$key] == null) {
			die('Inputkan subject dan persen');
		}
	}
	$sql_delete = "DELETE FROM `program_detail` WHERE `id_program` = '$id'";
	mysqli_query($GLOBALS['link'], $sql_delete) or die($sql_delete);
	foreach ($subject as $key => $value) {
		$sql_insert = "INSERT INTO `program_detail`(`id_program`, `id_subject`, `percent`) VALUES ('$id','" . sqlValue($value) . "','" . sqlValue($percent[$key]) . "')";
		mysqli_query($GLOBALS['link'], $sql_insert) or die($sql_insert);
	}
...
function showClassAdmin($id_cust)
{
	$sql = "SELECT a.* FROM exam_class a WHERE a.id_customer = '$id_cust'";
	$res = mysqli_query($GLOBALS['link'], $sql) or die($sql);
...
{
	$sql = "SELECT a.id_voucher,b.cust_name,c.program_name,d.available_val,a.type_voucher,c.id_program,c.pass_grade,c.duration
...
function planVoucher($id_voucher, $quota)
{
	...
	$result = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link'])); //voucher yang tersedia
	return $result;
}

function equSession($idg)
{
	$sql = "SELECT COUNT(a.id_student) cnt FROM exam_session a WHERE a.exam_code='$idg' and SUBSTRING_INDEX(a.id_student,'.',-1)=1";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
	$row_ses = mysqli_fetch_array($res);
	$count = $row_ses[0];

	return $count; // jumlah session yang mengikuti ujian
}

function createLog()
{
	list($dn, $tn) = timeVar();
	$sql = "SELECT a.date,a.start_time, a.id_schedule,a.exam_group,a.token,b.count_stu, b.id_voucher,a.status
	FROM exam_schedule a INNER JOIN exam_group b ON a.exam_group = b.exam_code
	WHERE a.status = 'init' or a.status = 'run'";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
	$setdate = date('Y-m-d', strtotime($dn));
	$set = date('H:i:s', strtotime($tn));
	while ($row = mysqli_fetch_array($res)) {
		$addtime = date('H:i:s', strtotime('+15 minutes', strtotime($row[1])));
		$adddate = date('Y-m-d', strtotime($row[0]));
		if ($adddate < $setdate or ($setdate == $adddate and $addtime <= $set)) {
			...
			if ($run == true) {
				cntDeff($idv, $cst, $stn, $n); //mengembalikan sisa voucher
				reducePlan($idv, $cst); //function untuk mereset alokasi voucher exam
			}
			if ($row[7] == 'init') {
				$qin = "UPDATE exam_schedule SET status = 'Expired' WHERE status='init' AND id_schedule = $ids";
				$run = mysqli_query($GLOBALS['link'], $qin);
			}
			// reducePlan($idv,$cst);
		}
...
function cntDeff($idv, $cst, $stn, $n)
{
	...
	$r = upVoucher($idv, $a); //sisa voucher dikembalikan
}
...
{
	$sql = "SELECT a.id_schedule,a.exam_group,a.date,a.start_time,a.duration,a.classroom,a.proctor,a.`status`,a.token, SUBSTRING_INDEX(SUBSTRING_INDEX(b.group_name,'.',2),'.',-1),b.count_stu,b.id_voucher,b.group_name from exam_schedule a INNER JOIN exam_group b ON a.exam_group=b.exam_code WHERE id_schedule = '$id'";
	$result = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));

	return $result;
}
function covideditExamSchedule($id)
{
	$sql = "SELECT a.id_schedule,a.exam_group,a.date,a.start_time,a.duration,a.classroom,a.proctor,a.`status`,a.token, SUBSTRING_INDEX(SUBSTRING_INDEX(b.group_name,'.',2),'.',-1),b.count_stu,b.id_voucher,b.group_name from covid_exam_schedule a INNER JOIN exam_group b ON a.exam_group=b.exam_code WHERE id_schedule = '$id'";
	$result = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
...
function maxParticipant($max, $id_program)
{
	$idusr = $_SESSION['admin_group'];
	$sql = "SELECT COUNT(a.idstudents) FROM students a INNER JOIN user_test b ON a.idstudents = b.user_id
	WHERE b.prioritas > 1 AND eff_date IS NOT NULL AND b.exam_code IS NULL AND SUBSTRING_INDEX(a.our_id,'.',1) = '$idusr'  AND b.id_program = '$id_program'";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
	$n = mysqli_fetch_array($res);
	$val = $n[0] + $max;
	return $val;
}
function maxParticipantAdmin($max, $id_program, $idusr)
{
	$sql = "SELECT COUNT(a.idstudents) FROM students a INNER JOIN user_test b ON a.idstudents = b.user_id
	WHERE b.prioritas > 1 AND eff_date IS NOT NULL AND b.exam_code IS NULL AND SUBSTRING_INDEX(a.our_id,'.',1) = '$idusr'  AND b.id_program = '$id_program'";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
...
function CovidcreateSchedule($date, $time, $duration, $proctor, $exam_id, $class)
{
	$sql = "INSERT INTO `covid_exam_schedule`( `exam_group`, `date`, `start_time`, `duration`, `proctor`, `status`,`classroom`)
		VALUES ($exam_id,'" . sqlValue($date) . "','" . sqlValue($time) . "','" . sqlValue($duration) . "','" . sqlValue($proctor) . "','init','" . sqlValue($class) . "')";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
...
function CovidupdateSchedule($date, $time, $duration, $proctor, $exam_id, $class, $id_schedule)
{
	$sql = "UPDATE covid_exam_schedule SET
		`date` =  '" . sqlValue($date) . "',
		`start_time` = '" . sqlValue($time) . "',
		 `proctor` = '" . sqlValue($proctor) . "',
		 `classroom` = '" . sqlValue($class) . "'
...
{
	$sql = "SELECT a.id_voucher,b.cust_name,c.program_name,d.available_val,c.duration,a.type_voucher,c.id_program
	FROM transact_voucher a INNER JOIN customer b ON a.id_customer=b.id_customer
	INNER JOIN programs c ON a.id_program=c.id_program
...
{
	//show quota
	$sql = "SELECT * FROM voucher_history WHERE user_id = '$student_id' and exam_code = '$exam_code'";
	$rs = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error('session error : ' . $GLOBALS['link']));
	if (!mysqli_num_rows($rs)) {
		...
		$new_quota = $row[0] - 1;
		$sqlupdate = "UPDATE tmp_voucher SET plan_val=$new_quota WHERE `id_voucher`= '$voucher_id'";
		mysqli_query($GLOBALS['link'], $sqlupdate) or die(mysqli_error($GLOBALS['link']));
		$saldo = $row[1] + $new_quota;
		$sql = "INSERT INTO `voucher_history`(`id_voucher`, `exam_code`, `status`, `date`, `user_id`,saldo)
				VALUES ('" . $voucher_id . "', $exam_code,'Usage', " . sqlFormatDate(date('m/d/Y H:i:s')) . ",'" . $student_id . "',$saldo)";
...
function CovidshowExam($id, $where)
{
	if (isset($where)) {
		$sql = "SELECT a.id_schedule,a.date,a.start_time,c.fname, a.token,a.status, b.count_stu ,a.exam_group
		FROM covid_exam_schedule a INNER JOIN exam_group b on a.exam_group=b.exam_code
...
function showExamAdmin($id, $where)
{
	if (isset($where)) {
		$sql = "SELECT a.id_schedule,a.date,a.start_time,c.fname, a.token,a.status, b.count_stu ,a.exam_group,a.notif
		FROM exam_schedule a INNER JOIN exam_group b on a.exam_group=b.exam_code
		INNER JOIN users c on a.proctor=c.id_user where c.cust_group ='" . sqlValue($id) . "' $where ";
	} else {
		$sql = "SELECT a.id_schedule,a.date,a.start_time,c.fname, a.token,a.status, b.count_stu ,a.exam_group,a.notif
		FROM exam_schedule a INNER JOIN exam_group b on a.exam_group=b.exam_code
		INNER JOIN users c on a.proctor=c.id_user where c.cust_group ='" . sqlValue($id) . "' ORDER BY a.id_schedule DESC";
...
function CovidshowExamProctor($id, $where)
{
	$idp = $_SESSION['admin_id'];
	if (isset($where)) {
		$sql = "SELECT a.id_schedule,a.date,a.start_time,c.fname, a.token,a.status, b.count_stu
...
function reportAdmin($desc, $examCode, $admin_id, $arrayExam)
{
	$time = timeVar();
	$sql = "INSERT INTO `exam_report`( `description`, `last_update`, `exam_code`, `created`) VALUES ('$desc','" . $time[0] . ' ' . $time[1] . "','$examCode','$admin_id')";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
	foreach ($arrayExam as $key => $value) {
		$sql = "UPDATE exam_schedule SET notif=1 WHERE id_schedule=$value";
		$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
	}
}
function paramReport($id)
{
	$time = timeVar();
	$sql = "SELECT * FROM exam_report WHERE id_report =$id";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
	$data = mysqli_fetch_array($res);
	$idSchedule = explode(',', $data[4]);
	foreach ($idSchedule as $key => $value) {
		if ($value != null) {
			$sqlSchedule = editExamSchedule($value);
			$dataSchedule = mysqli_fetch_array($sqlSchedule);
			if ($key == 0) {
				$bottomDate = $dataSchedule[2];
				$topDate = $dataSchedule[2];
			} else {
				if ($bottomDate > $dataSchedule[2]) {
					$bottomDate = $dataSchedule[2];
				}
				if ($topDate < $dataSchedule[2]) {
					$topDate = $dataSchedule[2];
				}
			}
		}
	}
	$id_cust = explode('.', $dataSchedule[6]);

	$data = array("$bottomDate", "$topDate", $data[1], $dataSchedule[11], $id_cust[0]);
	return $data;
}
...
function CovidshowExamDashProc($id)
{
	$idu = $_SESSION['admin_id'];
	$sql = "SELECT a.id_schedule,a.date,a.start_time,c.fname, a.token,a.status, b.count_stu
		FROM covid_exam_schedule a INNER JOIN exam_group b on a.exam_group=b.exam_code
...
{
	$sql = 'SELECT a.id_schedule,a.date,a.start_time,c.fname, a.token,a.status, b.count_stu,d.cust_name,a.notif
		FROM exam_schedule a INNER JOIN exam_group b on a.exam_group=b.exam_code
		INNER JOIN users c on a.proctor=c.id_user
		INNER JOIN customer d on d.id_customer = c.cust_group where d.id_customer != "C0002" ORDER BY a.date DESC LIMIT 700';
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));

	return $res;
}
function CovidshowAllExam()
{
	$sql = 'SELECT a.id_schedule,a.date,a.start_time,c.fname, a.token,a.status, b.count_stu,d.cust_name,a.notif
		FROM covid_exam_schedule a INNER JOIN exam_group b on a.exam_group=b.exam_code
		INNER JOIN users c on a.proctor=c.id_user
		INNER JOIN customer d on d.id_customer = c.cust_group where d.id_customer != "C0002" ORDER BY a.id_schedule DESC LIMIT 700';
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
...
{
	$sql1 = "select * from exam_schedule where id_schedule=$id";
	$res1 = mysqli_query($GLOBALS['link'], $sql1) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql1);
	$row = mysqli_fetch_array($res1);
	if ($row[8] == null) {
		$row_time = strtotime("+15 minutes", strtotime($row[3]));
		$time15 = date('H:i:s', $row_time);
		if ($time15 >= $time) {
			$sql = "UPDATE exam_schedule SET start_time='$time', status='run', token='" . sqlValue($token) . "' where id_schedule=$id";
			$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
			$res = $token;
		} else {
			$res = "<p style=\"font-size: 18px;text-shadow: 0px 0px;color: #424242;\">Exam Schedule Expired<br> Please Create New Exam Schedule</p>";
		}
	} else {
		$res = $row[8];
	}
...
function covidupdateExamToken($token, $id, $time)
{
	$sql1 = "select * from covid_exam_schedule where id_schedule=$id";
	$res1 = mysqli_query($GLOBALS['link'], $sql1) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql1);
	$row = mysqli_fetch_array($res1);
	if ($row[8] == null) {
		$row_time = strtotime("+15 minutes", strtotime($row[3]));
		...
		$dateNow = date('Y-m-d');
		if ($row[2] >= $dateNow) {
			$sql = "UPDATE covid_exam_schedule SET  status='run', token='" . sqlValue($token) . "' where id_schedule=$id";
			$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
			$res = $token;
		} else {
			$res = "<p style=\"font-size: 18px;text-shadow: 0px 0px;color: #424242;\">Exam Schedule Expired<br> Please Create New Exam Schedule</p>";
...
function examPause($id)
{
	$sql = "UPDATE exam_schedule SET status='pause' where id_schedule=$id";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);

	return $res;
}
function examContinue($id)
{
...
{
	$time = timeVar();
	$date = date('Y-m-d H:i:s', strtotime($time[0] . ' ' . $time[1]));
	$sql = "SELECT id_schedule,date,start_time,duration,proctor from exam_schedule WHERE status='run' ";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
	while ($data = mysqli_fetch_array($res)) {
		$sch = date('Y-m-d H:i:s', strtotime($data['date'] . ' ' . $data['start_time']));
		$duration = $data['duration'] + 60;
		$sch_expire = date('Y-m-d H:i:s', strtotime('+' . $duration . 'minutes', strtotime($sch)));
		if ($date > $sch_expire) {
			$idCustomer = explode('.', $data['proctor']);
			if ($idCustomer[0] == 'C0002') {
				$sql = "UPDATE exam_schedule SET status = 'finish' WHERE status='run' AND id_schedule = $data[0]";
			} else {
				$sql = "UPDATE exam_schedule SET status = 'finish',notif = 0 WHERE status='run' AND id_schedule = $data[0]";
			}
			$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
		}
	}
}
function CovidautoFinish()
{
	$time = timeVar();
	$sql = "SELECT id_schedule,date,start_time,duration,proctor from covid_exam_schedule WHERE status='run' ";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
	while ($rowExam = mysqli_fetch_array($res)) {
		$time_limit = $rowExam[3] + 60;
		$rowTime = strtotime("+" . $time_limit . " minutes", strtotime($rowExam[2]));
		$endTime = date('H:i:s', $rowTime);
		//echo $time[1].'>'.$endTime;
		if ($time[0] > $rowExam[1]) {
			//echo "on";
			$idCustomer = explode('.', $rowExam[4]);
			if ($idCustomer[0] == 'C0002') {
				$sql = "UPDATE covid_exam_schedule SET status = 'finish' WHERE status='run' AND id_schedule = $rowExam[0]";
			} else {
				$sql = "UPDATE covid_exam_schedule SET status = 'finish',notif = 0 WHERE status='run' AND id_schedule = $rowExam[0]";
			}
			$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
		}
	}
...
{
	$sql = "SELECT a.id_session,a.start_time,a.exam_code,SUBSTRING_INDEX(SUBSTRING_INDEX(b.group_name,'.',2),'.',-1) idpro, c.duration,a.id_student
	...
	while ($dat = mysqli_fetch_array($res)) {
		...
		$datetime_limit = date('Y-m-d H:i:s', strtotime('+' . $duration + 10 . ' minutes', strtotime($dat[1])));
		//echo($id_ses);
		if ($curdate > $date or $datetime_now > $datetime_limit) {
			forceEndExam($dat[5], $dat[2]);
...
function ForceActivateNextId($user_id, $program)
{
	...
	if ($row = mysqli_fetch_row($rs)) {
		...
		$sql = " select id_peserta from user_test where user_id ='" . $user_id . "' and exam_code is null and id_program='$program' order by prioritas,id_peserta";
...
function forceEndExam($id_peserta, $exam_group)
{
	$id_student = explode('.', $id_peserta, -1);
	$student_data = getDataStudent($id_student[0]);
	$sql = "select a.exam_code,a.id_voucher,c.pass_grade,c.margin,b.id_program from exam_group a join transact_voucher b on a.id_voucher=b.id_voucher join programs c on b.id_program=c.id_program where a.exam_code=$exam_group";
	$rs = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
	...
	if ($percent < $persen_lulus) {
		ForceActivateNextId($id_student[0], $id_program);
	}
...
function showStueExam($id)
{
	...
	INNER JOIN exam_session b ON a.exam_group=b.exam_code WHERE a.exam_group='$id' and SUBSTRING_INDEX(b.id_student,'.','-1')=1";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
	...
	<table class="table table-xs table-bordered" id="tbStudents"><thead><tr>
		...
		$table .= '<script> $(document).ready(function(){ $("#tbStudents").DataTable({ "bLengthChange": false,"paging": false});});</script>';
	}
	$sql = "SELECT SUBSTRING_INDEX(b.id_student,'.','1') n, b.start_time, b.end_time FROM exam_schedule a
	INNER JOIN exam_session b ON a.exam_group=b.exam_code WHERE a.exam_group='$id' and SUBSTRING_INDEX(b.id_student,'.','-1')!=1";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
	...
	<table class="table table-xs table-bordered" id="tbStudentsRe"><thead><tr>
		<th scope = "col" > No </th>
		<th scope = "col" > Students ID </th>
		<th scope = "col" > Name </th>
		<th scope = "col" > Start Exam </th>
		...
		$table .= '</tbody></table>';
		$table .= '<script> $(document).ready(function(){ $("#tbStudentsRe").DataTable({"bLengthChange": false,"paging": false});});</script>';
	}

	return $table;
}
function CovidshowStueExam($id)
{
	$table='';
	$sql = "SELECT SUBSTRING_INDEX(b.id_student,'.','1') n, b.start_time, b.end_time FROM covid_exam_schedule a
	INNER JOIN exam_session b ON a.exam_group=b.exam_code WHERE a.exam_group='$id' and SUBSTRING_INDEX(b.id_student,'.','-1')=1";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
	if (mysqli_num_rows($res)) {
		$table .=
	...
	<table class="table table-xs table-bordered" id="tbStudents"><thead><tr>
		...
		$table .= '</tbody></table><br>';
		$table .= '<script> $(document).ready(function(){ $("#tbStudents").DataTable({ "bLengthChange": false});});</script>';
	}
	$sql = "SELECT SUBSTRING_INDEX(b.id_student,'.','1') n, b.start_time, b.end_time FROM covid_exam_schedule a
	INNER JOIN exam_session b ON a.exam_group=b.exam_code WHERE a.exam_group='$id' and SUBSTRING_INDEX(b.id_student,'.','-1')!=1";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
	...
	<table class="table table-xs table-bordered" id="tbStudentsRe"><thead><tr>
		...
		$table .= '</tbody></table>';
		$table .= '<script> $(document).ready(function(){ $("#tbStudentsRe").DataTable({"bLengthChange": false});});</script>';
	}
...
{
	$sql = "SELECT  sum(percentage_true),(sum(a.percentage_true)+sum(a.percentage_false)+sum(a.percentage_null)) total FROM exam_percentage a inner join subject_ls b on a.id_subject = b.id_subject
	WHERE SUBSTRING_INDEX(a.id_student,'.',1) = '$ids' AND a.exam_code='$idg' group by b.subject_group";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
...
{
	$sql = "SELECT a.id_program, b.subject_name, b.subject_group
	FROM program_detail a INNER JOIN subject_ls b ON a.id_subject= b.id_subject WHERE a.id_program = '$id' group by b.subject_group";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
	return $res;
}

function repStue($id_g, $id, $a, $b)
{
	...
	AND a.date BETWEEN '$a' AND '$b' AND SUBSTRING_INDEX(b.group_name,'.','1')='$id_g' AND `status`!='init')";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);

	return $res;
}
function CovidrepStue($id_g, $id, $a, $b)
{
	$sql = "SELECT bb.exam_code,SUBSTRING_INDEX(bb.id_student,'.',1) ids, DATE_FORMAT(bb.start_time,'%Y-%m-%d') date_id  FROM exam_session bb
	WHERE bb.exam_code IN
	(SELECT a.exam_group FROM covid_exam_schedule a INNER JOIN exam_group b ON a.exam_group = b.exam_code
	WHERE SUBSTRING_INDEX(SUBSTRING_INDEX(b.group_name,'.','2'),'.','-1') ='$id'
	AND a.date BETWEEN '$a' AND '$b' AND SUBSTRING_INDEX(b.group_name,'.','1')='$id_g' AND `status`!='init')";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
...
function groupReport($id_g, $name, $date_1, $date_2, $program)
{
	...
	if ($view[2] == '2') {
		...
		<tr>';
			...
			$table .= '</tr>';
			$idstu = repStue($id_g, $rp[0], $date_1, $date_2);
			$nom = 1;
...
function CovidgroupReport($id_g, $name, $date_1, $date_2, $program)
{
	$id = $_SESSION['admin_group'];
	$date_1 = $date_1;
	$date_2 = $date_2;
	...
	if ($program != 1) {
		$sql1 = "SELECT id_program FROM programs WHERE id_program = '" . $program . "'";
	} else {
		$sql1 = "SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(a.group_name,'.','2'),'.','-1') id_program
	 FROM exam_group a INNER JOIN covid_exam_schedule b ON a.exam_code = b.exam_group INNER JOIN
	 (SELECT aa.exam_code FROM exam_session aa GROUP BY aa.exam_code) c ON a.exam_code=c.exam_code
	 WHERE
	 b.date BETWEEN '$date_1' AND '$date_2' GROUP BY SUBSTRING_INDEX(SUBSTRING_INDEX(a.group_name,'.','2'),'.','-1')";
	}
	...
	if ($view[2] == '2') {
		...
		<tr>';
			...
			$table .= '</tr>';
			$idstu = CovidrepStue($id_g, $rp[0], $date_1, $date_2); //mencari peserta
			$nom = 1;
			while ($r = mysqli_fetch_array($idstu)) {
				$name = neStu($r[1]); //mencari nama peserta
				$nilai = filterValue($r[1], $r[0]);
				$table .= '<tr>';
				$table .= '<td>' . $nom . '</td>';
				$table .= '<td>Group ' . $r[0] . '</td>';
		...
		<tr>';
			...
			$table .= '</tr>';
			$idstu = CovidrepStue($id_g, $rp[0], $date_1, $date_2);
			$nom = 1;
			while ($r = mysqli_fetch_array($idstu)) {
				$name = neStu($r[1]);
				$nilai = filterValue($r[1], $r[0]);
...
function exeFile($title)
{
	...
	} elseif ($title == 'program_exam_at_home') {
		$par = '7_pic_export.php';
	} else {
		$par = null;
	}
...
function lastReport($amn)
{
	...
	WHERE SUBSTRING_INDEX(a.id_report,'.',1) = '$id' ORDER BY CONVERT(SUBSTRING_INDEX(a.id_report,'.',-1),UNSIGNED INTEGER) DESC";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
...
function progReport($id, $a, $b)
{
	...
	if ($id_prog != 1) {
		...
		WHERE b.`status` != 'init' AND c.id_program='$id_prog' AND b.date BETWEEN '$data_a' AND '$data_b' AND SUBSTRING_INDEX(a.group_name,'.',1) = '$idses' GROUP BY c.id_program";
	} else {
		$sql = "SELECT a.exam_code,c.program_name,c.sum_question,c.pass_grade,c.id_program
		FROM exam_group a INNER JOIN exam_schedule b ON a.exam_code=b.exam_group LEFT JOIN programs c ON SUBSTRING_INDEX(SUBSTRING_INDEX(a.group_name,'.',2),'.',-1)=c.id_program
		WHERE b.`status` != 'init' AND b.date BETWEEN '$data_a' AND '$data_b' AND SUBSTRING_INDEX(a.group_name,'.',1) = '$idses' GROUP BY c.id_program";
	}
