<?php
function cek_athome_config($id_customer)
{
	$sql = "SELECT * FROM covid_customer_config where id_customer = '$id_customer'";
	$res = mysqli_query($GLOBALS['link'], $sql);
	$result = mysqli_fetch_array($res);

	return $result;
}
function set_athome_config($id_customer, $set)
{
	$sql = "UPDATE covid_customer_config SET at_home = $set where id_customer = '$id_customer'";
	//die($sql);
	$res = mysqli_query($GLOBALS['link'], $sql);
	$result = mysqli_fetch_array($res);
...
{
	$sql = "SELECT a.*,c.subject_name,b.percent,c.subject_group FROM programs a left join program_detail b on a.id_program=b.id_program left join subject_ls c on b.id_subject=c.id_subject WHERE a.id_program = '$id'";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
	return $res;
}
function detailProgramsAdmin($id)
{
	$sql = "SELECT a.*,c.subject_name,sum(b.percent),c.subject_group FROM programs a inner join program_detail b on a.id_program=b.id_program inner join subject_ls c on b.id_subject=c.id_subject WHERE a.id_program = '$id' group by c.subject_group";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
...
{
	$sql = "SELECT b.id_subject,b.subject_name,b.level,a.percent,a.id
...
{
	$sql = 'SELECT a.id_subject,a.subject_name,a.level,a.subject_group FROM  subject_ls a ';
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));

	return $res;
}
function addSubject($id, $name, $level, $subject_group)
{
	...
	$sql = "INSERT INTO `subject_ls`(`id_subject`, `subject_name`, `level`,subject_group) VALUES ('" . sqlValue($new_id) . "','" . sqlValue($name) . "','" . sqlValue($level) . "','" . sqlValue($subject_group) . "')";
...
function addVoucher($customer, $program, $amount, $today, $type, $inv_num, $inv_date)
{
	...
	$sql = "INSERT INTO `voucher_history`(`id_voucher`,`exam_code`,`status`, `date`, `top_up`,saldo,inv_num,inv_date,user_id) 
	VALUES ('$new_id','0','Topup','$today',$amount,$amount,'$inv_num','$inv_date','$user_admin')";
	mysqli_query($GLOBALS['link'], $sql) or die($sql);
}
...
function historyReExamDetail($id, $exam_code)
{
	$sql = "SELECT b.id_student,'.','1', b.start_time, b.end_time FROM exam_schedule a
	INNER JOIN exam_session b ON a.exam_group=b.exam_code WHERE a.exam_group='$exam_code' and SUBSTRING_INDEX(b.id_student,'.','-1')!=1";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
...
function topUpVoucher($id, $topup, $inv_num, $inv_date)
{
	...
	$user_admin = $_SESSION['admin_id'];
	// 
	$sql_a = "UPDATE transact_voucher SET lastest_topup = '$today' where id_voucher='$id'";
	mysqli_query($GLOBALS['link'], $sql_a) or die($sql_a);
	// 
	$sql_b = "UPDATE  tmp_voucher SET available_val = available_val+'$topup' where id_voucher='$id'";
	mysqli_query($GLOBALS['link'], $sql_b) or die($sql_b);
	// 
	$sql1 = "SELECT available_val,plan_val FROM tmp_voucher  WHERE id_voucher='$id'";
	$exe_sql1 = mysqli_query($GLOBALS['link'], $sql1) or die($sql1);
	$val_sql = mysqli_fetch_array($exe_sql1);
	$voucher_balance = $val_sql[0] + $val_sql[1];
	// 
	$sql = "INSERT INTO `voucher_history`(`id_voucher`,`exam_code`,`status`, `date`, `top_up`,`saldo`,`inv_num`,`inv_date`,`user_id`) 
		VALUES ('$id','0','Topup','$today','$topup','$voucher_balance','$inv_num','$inv_date','$user_admin')";
	mysqli_query($GLOBALS['link'], $sql) or die($sql);
}
...
function CovidlistExamParticipants($group)
{
...
function ListUserRemidial()
{
	$sql = "select * from students_remidial where submited = 'Y'";
	$rs = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
...
function approveUserRemidial($nim, $user, $prog, $cust_group, $margin)
{
	foreach ($user as $key => $value) {
		...
		$qwer = "SELECT max(prioritas),max(notif) FROM user_test WHERE user_id = '$value' AND id_program='$prog[$key]'";
		...
		for ($i = 1; $i <= $margin[$key]; ++$i) {
			if ($i == 1) {
				...
				$sql = "INSERT INTO user_test(user_id,id_peserta,status,prioritas,notif,eff_date,id_program) 
				values ('" . $nim[$key] . "','" . $userTest . "',1," . ($i + $dataUser[0]) . ',' . ($dataUser[1] + 1) . ',' . $skrg . ",'" . sqlValue($prog[$key]) . "')";
			} else {
				$userTest = $value . '.' . $i;
				$sql = "INSERT INTO user_test(user_id,id_peserta,status,prioritas,notif,eff_date,id_program) 
				values ('" . $nim[$key] . "','" . $userTest . "',0," . ($i + $dataUser[0]) . ',' . ($dataUser[1] + 1) . ",null,'" . sqlValue($prog[$key]) . "')";
			}
			mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
		}
		// $qwer = "DELETE FROM students_remidial WHERE nim = '$nim[$key]' AND program='$prog[$key]'";
		$exe = mysqli_query($GLOBALS['link'], $qwer) or die(mysqli_error($GLOBALS['link']));
	}
}
...
function CovidexamStudentLogin($id_peserta, $password)
{
	//mencocokan id peserta dan password apa 1 group
	//tambahkan where jadwal ujian masih belum kadaluarsa
	//ketentuan : start_time + 15 menit > date now
	$sql = "SELECT b.idstudents,b.fname,SUBSTRING_INDEX(b.our_id,'.','1') ,a.date,a.start_time, a.exam_group, a.id_schedule , c.id_voucher
	FROM covid_exam_schedule a INNER JOIN students b ON SUBSTRING_INDEX(b.our_id,'.','1') = SUBSTRING_INDEX(a.proctor,'.','1') INNER JOIN exam_group c ON a.exam_group = c.exam_code
	WHERE b.idstudents='" . sqlValue($id_peserta) . "' AND a.status='run' AND a.start_time <= CURRENT_TIME() AND a.token ='" . sqlValue($password) . "'";
	$rs = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
	if ($row = mysqli_fetch_row($rs)) {
		$GLOBALS['exam_group'] = $row[5];
...
function studentOnlineCek_arr($ids)
{
	// $data = array();
	$idList = '"'.implode('","', $ids).'"';
	$sql = "SELECT * FROM online_student WHERE id_students IN ($idList) AND logout_time IS NULL";
	$rs = mysqli_query($GLOBALS['link'], $sql);
	while ($row = mysqli_fetch_array($rs)) {
		$data[] = $row['id_students'];
	}
	return $data;
}
...
function autologLogout()
{
	$sql = "select * from online_student where logout_time is null";
	$rs = mysqli_query($GLOBALS['link'], $sql);
	$skrg = date('m/d/Y H:i:s');
	while ($row = mysqli_fetch_row($rs)) {
		$logout = date('m/d/Y H:i:s', strtotime($row[2]) + (3 * 60 * 60));
		if ($skrg >= $logout) {
			$sql = 'update online_student set logout_time = ' . sqlFormatDate($skrg) . " where id_students='" . $row[1] . "'and logout_time is null";
			mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
		}
...
function cekSessionExist()
{
	...
	$sql1 = "select a.id_session,a.exam_code from exam_session a inner join exam_group b on a.exam_code=b.exam_code where a.id_student = '" . sqlValue($id_peserta) . "' AND b.id_voucher=(select id_voucher from exam_group where exam_code = $exam_group) ORDER BY a.id_session desc";
	$rs1 = mysqli_query($GLOBALS['link'], $sql1) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql1);
	$sumParticipant = mysqli_num_rows($rs1);
	if ($dataUserTest[0] >= $sumParticipant) {
		return false;
	} else {
...
{ // untuk menghitung ujian user sudah remidial
	...
	// $sql1 = "select count(a.id_student) from exam_participants a inner join exam_group b on a.exam_group=b.exam_code 
	//          inner join exam_session c on a.id_student = c.id_student and a.exam_group = c.exam_code
	//          where a.id_student = '".sqlValue($id_peserta)."' AND b.id_voucher=(select id_voucher from exam_group where exam_code = $exam_group) and b.exam_code !=$exam_group";
	$rs1 = mysqli_query($GLOBALS['link'], $sql1) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql1);
...
{
	global $id_peserta, $exam_group;
	$sql = "select a.exam_group,a.id_student,b.id_voucher,c.id_program,d.program_name from exam_participants a inner join exam_group b on a.exam_group=b.exam_code inner join transact_voucher c on b.id_voucher = c.id_voucher inner join programs d on c.id_program=d.id_program where a.id_student= '" . sqlValue($id_peserta) . "' AND a.exam_group=$exam_group";
	$rs = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
...
function checkJadwal($exam_code)
{
	$sql = "SELECT start_time FROM exam_schedule a WHERE a.status='run' AND exam_group = '$exam_code'";
	$rs = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '$sql');
	$res = mysqli_fetch_array($rs);
	$row_limit_start = strtotime('+15 minutes', strtotime($res[0]));
	$limit_start = date('H:i:s', $row_limit_start);
	$timeNow = date('H:i:s');
	if ($limit_start > $timeNow) {
		//echo "true";
		return true;
	} else {
		//echo "false";
...
function checkVoucher($idV)
{
	$sql = "SELECT a.id_voucher,a.type_voucher,b.saldo, b.date FROM transact_voucher a inner join voucher_history b on a.id_voucher = b.id_voucher
		WHERE a.id_voucher ='$idV' 
...
function generateQuestion($program_id, $exam_code, $student_id)
{
	...
	while ($row = mysqli_fetch_row($rs)) {
		...
		$duration[] = $row[1];
		$subs[] = $row[3];
		$r[] = $row[4]; //jumlah soal per subject
		++$i;
	}
	//random soal
	$no_quest = 1;
...
{
	$sql = "SELECT * FROM exam_session WHERE id_student = '$id_peserta' and exam_code = '$exam_code'";
...
function showReportListAdmin($extra)
{
	if (isset($extra)) {
		$sql = "SELECT * FROM exam_report $extra";
	} else {
		$sql = "SELECT * FROM exam_report";
	}
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));

	return $res;
}
function updateStatusReport($id, $status)
{
	$time = timeVar();
	$sql = "UPDATE exam_report SET status='$status', last_update = '" . $time[0] . " " . $time[1] . "' WHERE id_report = $id ";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
	reportHistory($id, $status);
}
function reportHistory($id, $status)
{
	$time = timeVar();
	$sql = "insert into exam_report_history (id_report,status,datetime,user_id) value ($id,'$status','" . $time[0] . " " . $time[1] . "','" . $_SESSION['admin_id'] . "')";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
}
function showHistoryReport($id)
{
	$time = timeVar();
	$sql = "SELECT a.*,b.fname FROM exam_report_history a left join users b on a.user_id = b.id_user where a.id_report = $id ORDER BY a.datetime ";
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));
	return $res;
}
...
function admRpt_Exam($start, $end, $exam)
{
	...
	//        a.start_time,
	//        a.end_time,
	//        sum(d.percentage_true)benar,
	//        sum(d.percentage_true+d.percentage_false+d.percentage_null)soal,
	//        sum(d.percentage_true)/sum(d.percentage_true+d.percentage_false+d.percentage_null)*100 nilai,
	//        e.pass_grade, 
	//        c.idstudents 
	//    from exam_session a
	...
	//and c.date >='$start' and c.date <= '$end'
	//echo($sql);
	$sql = "SELECT b.group_name,c.fname, c.idstudents ,a.exam_code,sum(d.percentage_true)/sum(d.percentage_true+d.percentage_false+d.percentage_null)*100 nilai,e.pass_grade,a.id_student,a.start_time
	FROM exam_session a 
	inner join exam_group b on a.exam_code=b.exam_code 
	inner join students c on SUBSTRING_INDEX(a.id_student,'.',1)=c.idstudents 
	left join exam_percentage d on a.id_student = d.id_student and a.exam_code = d.exam_code
	inner join programs e on SUBSTRING_INDEX(SUBSTRING_INDEX(b.group_name,'.',2),'.',-1) = e.id_program
	where b.exam_code = '$exam'
	group by a.id_student,d.exam_code";
	// $sql = "SELECT AA.group_name, BB.fname, BB.idstudents,AA.exam_code,AA.nilai,CC.pass_grade,AA.id_student from
	//         (SELECT a.group_name,sum(b.percentage_true)/sum(b.percentage_true+b.percentage_false+b.percentage_null)*100 nilai , b.id_student,a.exam_code
	//         FROM exam_group a 
	//         inner join exam_percentage b on a.exam_code = b.exam_code 
	//         inner join programs c on SUBSTRING_INDEX(SUBSTRING_INDEX(a.group_name,'.',2),'.',-1) = c.id_program 
	//         where a.group_name = '$exam' 
	//         group by a.exam_code,b.id_student) AA 
	//         INNER JOIN students BB on SUBSTRING_INDEX(AA.id_student,'.',1) = BB.idstudents and SUBSTRING_INDEX(AA.group_name,'.',1) = SUBSTRING_INDEX(BB.our_id,'.',1)
	//         INNER JOIN programs CC on SUBSTRING_INDEX(SUBSTRING_INDEX(AA.group_name,'.',2),'.',-1) = CC.id_program";
	//echo $sql;
	$res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']));

	return $res;
}
function PicReportExam($start, $end, $exam)
{
	$len = count($exam);
	foreach ($exam as $key => $value) {
		if ($key < $len - 1) {
