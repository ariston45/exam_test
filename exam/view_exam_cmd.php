...
//questions maps
		...
		<div class=\"col-md-2\">
          <div class=\"panel panel-default chat\">
            ...
            <div class=\"panel-body\" style=\"padding: 5px;display: flex;flex-wrap: wrap;\">
              ";
              	$x=1;
                while ($row = mysqli_fetch_row($rs)) {
                	if ($x==$nomorSoal) {
                		echo '<button type="button" id="no'.$x.'" style="background-color: #2e6da4;color: white; max-height: 40px;" class="btn btn-outline btn-primary btn-sm btn-number" onclick="formSubmitNumber('.$x.')" >'.$x.'. '.$row[1].'</button>&nbsp;';
                	} else if ($row[0]=='True'){
                   	 	echo '<button type="button" id="no'.$x.'" style="background-color: #ffbc00; max-height: 40px;" class="btn btn-outline btn-primary btn-sm btn-number" onclick="formSubmitNumber('.$x.')" >'.$x.'. '.$row[1].'</button>&nbsp;';
                	} else if ($row[0]=='False'){
                    	echo '<button type="button" id="no'.$x.'" style="background-color: #00b842;max-height: 40px;" class="btn btn-outline btn-primary btn-sm btn-number" onclick="formSubmitNumber('.$x.')" >'.$x.'. '.$row[1].'</button>&nbsp;';
                	} else {
                    	echo '<button type="button" id="no'.$x.'" style="max-height: 40px;" class="btn btn-outline btn-primary btn-sm btn-number" onclick="formSubmitNumber('.$x.')" >'.$x.'. '.$row[1].'</button>&nbsp;';
                	}
                $x++;
                }
...
// tombol navigasi
		echo "<form id=\"navigation\" action=\"\" method=\"POST\">
					<div class='col-lg-3' style='text-align :left; padding-left:0;'>
				 ";
		if ($row[10]=='True') {
			echo"<label class='lbl-checkbox'><input type='checkbox' id=\"mark\" name='mark' value='True' checked> Mark For Review</label>";
		}else{
			echo"<label class='lbl-checkbox'><input type='checkbox' id=\"mark\" name='mark' value='True'> Mark For Review</label>";
		}
		echo "
				</div>
	         ...
	         ";
        if ($nomorSoal > 1){
			echo "<button class = \"btn btn-sm btn-success\" onclick=\"formSubmitPrev()\" style=\"width: 110;\">Prev</button>&nbsp";
			//echo "<input class = \"btn btn-sm btn-success\" value=\"Prev\" onclick=\"formSubmitPrev()\">&nbsp";
		}
		if ($nomorSoal < $jumlahSoal){
			echo "<button class = \"btn btn-sm btn-success\" onclick=\"formSubmitNext()\"  style=\"width: 110;\">Next</button>";
	...
	//		echo "&nbsp<input class = \"btn btn-sm btn-primary\" value=\"Review\" onclick=\"formSubmitReview()\"> &nbsp";
			echo "<button class =\"btn btn-sm btn-primary\" onclick=\"formSubmitReview()\"> Review </button>	";
		}
				echo "
						</div>
...
function endSession($id_peserta,$exam_group){
	$sql="update exam_session set end_time=".sqlFormatDate(date("m/d/Y H:i"),"%m/%d/%Y %H:%i")."
				 where id_student='".$id_peserta."' and exam_code = '".$exam_group."'";
	//echo $sql;
	mysqli_query($GLOBALS['link'],$sql) or  die(mysqli_error($GLOBALS['link']));
...
$sql="SELECT subject,
    ...
    group by subject";
  $rs=mysqli_query($GLOBALS['link'],$sql) or die(mysqli_error($GLOBALS['link']));
	$viewresult.= "<div style='text-align:center; margin-top:20px;'>";
	$viewresult.= "<table>";
	$viewresult.= "<tr><td rowspan=2 width=270px> <img style=\"max-width: 270px; max-height:100px;\" src= '../assets/img/logo/".$student_data[2]."'> </td><th style='text-align: center;'>ID </th><th colspan=3>&nbsp&nbsp ".$student_data[1]."</th></tr>";
	$viewresult.= "<tr><th style='text-align: center;'>NAME </th><th colspan=3> &nbsp&nbsp".$student_data[0]."</th></tr>";
	$viewresult.= "<tr>
  ...
  <tr>
    ...
    <th style='text-align: center;'>Not Answer </th>
  </tr>";
  while ($row=mysqli_fetch_row($rs)){
    $benar+=$row[1];
    $soal+=$row[4];
//insert exam_percentage
  ...
  foreach(resultExam($student_data[1],$exam_group) as $r) {
	  $quest = $r[2] + $r[3] + $r[4];
	  $point[] = $quest;
	  $true[] = $r[2];
	  $viewresult.='
...
$result=mysqli_fetch_row($rs);
$persen_lulus=$result[2];
$sql="SELECT a.id, b.subject_group, sum(a.percentage_true), sum(a.percentage_false), sum(a.percentage_null),b.id_subject FROM exam_percentage a LEFT JOIN subject_ls b ON a.id_subject=b.id_subject WHERE SUBSTRING_INDEX(a.id_student,'.',1)='$id_peserta' AND a.exam_code='$exam_group' group by b.subject_group";
...
$conc = concResult($exam_group,$val);
echo'
  <tr>
...
function startPage(){
	...
	echo '
<div class="row ">
<div class="col-sm-12 " style="margin-top: 0;  background-color: #ffffff75; height: auto;">
  <div class="column-left" >
   <div class="notice">
    <div class="row contain-row">
      <div class="col-sm-6 col6-head"><img src="../assets/img/TR-ACADEMIC-150x38.png" style="padding-top:10px;"></div>
      <div class="col-sm-6 col6-head" style="text-align:right"><img src="../assets/img/logo/'.$logo[2].'" style="max-height: 70px;max-width: 135px;"></div>
    </div>
  ...
  <div class="column-right">
	...
	<table style="width:100%;height:135px;" >
	  ...
	  <tr>
		<td style="padding-bottom: 20px;" valign="top">PROGRAM</td>
		<td style="padding-bottom: 20px;" valign="top">:</td>
		<td style="padding-bottom: 20px;" valign="top">'.$row[4].'</td>
	  </tr>
	  <tr>
	  	<td colspan=3><p style="font-size: 11px;color: #d54f1b;">Setelah anda klik start, maka voucher ujian anda akan terpakai dan tidak dapat dikembalikan .</p></td>
	  </tr>
	</table>
	...
	<form id="form-id" action="" method="POST" >
		...
		<button type="submit" class="btn btn-danger btn-block">Start Exam</button>
	</form>
	<form id="form-id" action="" method="POST" >
		<input type="hidden" name="exam_group" value='.$row[0].'>
		<input type="hidden" name="participant_id" value='.$row[1].'>
