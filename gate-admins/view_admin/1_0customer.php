...
if (isset($_POST['cmd'])) {
	switch ($_POST['cmd']) {
		...
		//die('jan');
			...
			$at_home_config = $_POST['at_home_config'];
			set_athome_config($idcustomer,$at_home_config);
			break;
		case 'Add PIC':
			$id = $_POST['id'];
			$name = $_POST['name'];
					...
					<tbody>
						...
						while ($row = mysqli_fetch_array($result)) {
							...
							if (mysqli_num_rows($rs) > 0) {
								...
								$button_pic = '<a href="?pg=detail_customer&CC=' . $row[0] . '" class="btn btn-xs btn-info" style="padding-right: 10px;padding-left: 10px;"> <i class="fa fa-info-circle"></i> Detail</a>';
							} else {
							...
							if ($row[0] != 'C0000') {
								...
								<tr>
									...
									<button type="button" class="btn btn-xs btn-danger " data-id="' . $row[0] . '"  data-toggle="modal" data-target="#delete_cust"><i class="fa fa-trash"></i>  Delete</button>
									<button type="button" class="btn btn-xs btn-warning " data-id="' . $row[0] . '"  data-toggle="modal" data-target="#home_exam"><i class="fa fa-cogs"></i>  Home Exam</button></td>
								</tr>
								';
								$no++;
							}
...
<!-- Modal Set Home Exam -->
<div class="modal fade" id="home_exam" role="dialog">
	<div class="modal-dialog">
		<!-- Modal content-->
		<div class="modal-content">
...
<script>
	...
	$(document).ready(function() {
		$('#home_exam').on('show.bs.modal', function(e) {
			var rowid = $(e.relatedTarget).data('id');
			$.ajax({
				type: 'get',
				url: 'view_admin/1_home_exam.php', //Here you will fetch records
				data: 'id=' + rowid, //Pass $id
				success: function(data) {
					$('.fetched-data').html(data); //Show fetched data from database
				}
