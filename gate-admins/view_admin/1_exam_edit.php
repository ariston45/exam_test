  ...
  switch ($_POST['cmd']) {
      ...
      if ($insert_group) {
    	  updateSchedule($date,$time,$duration,$proctor,$insert_group[b],$class,$id_schedule);
			...
			echo '
			<script>window.location.href ="dashboard.php?pg=detail_customer&CC='.$_GET["id_g"].'"</script>
			';
			// header('location:?pg=pic_exam_schedule');
			// die();
	break;
...
$max_stue = maxParticipantAdmin($max, $row[5],$_GET['id_g']);
								...
								<div class="panel-body">
									...
									<div class="form-group">
										...
										<select class="form-control" name="class" required="" style="max-width: 750px;">
											<?php $result=showClassAdmin($_GET['id_g']);
									...
									<div class="form-group">
										...
										<select class="form-control" name="proctor" required="" style="max-width: 750px;">
											<?php $result=showProctor($_GET['id_g']);
											while($dat = mysqli_fetch_array($result)){
									...
									<br>
									<p class="text-right"> 
										<a href='?pg=detail_customer&CC=<?=$_GET["id_g"]?>' class="btn btn-warning btn-sm" > Cancel</a>
