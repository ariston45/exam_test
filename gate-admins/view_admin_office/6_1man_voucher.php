	...
	switch ($_POST['cmd']) {
		case 'Submit':
			...
			if (checkExistingVoucher($customer, $program, $type)) {
				...
				echo "	<script LANGUAGE='JavaScript'>window.alert(\"Duplicate Voucher\");</script>";
			}
			break;
		case 'Delete':
			$idv = $_POST['id'];
	...
	<div class="col-md-12">
		<div class="panel panel-info">
				...
				<table class="table table-bordered table-xs" id="tbVoucher">
					...
					<tbody>
						...
						while ($row = mysqli_fetch_array($result)) {
							...
							///////// +memberi value 0 jika voucher bernilai kurang dari 0
							if ($row[3] <= 0) {
								$val = 0;
							} else {
								$val = $row[3];
            ...
            <td>' . $row[1] . '</td>
						...
						<button type="button" class="btn btn-xs btn-primary" onclick="actionTopUp(\'' . $row[0] . '\')" ><i class="fa fa-shopping-cart"></i>  Transaction</button> &nbsp';
							if ($_SESSION['admin_level'] == 'Administrator') {
								echo '<button type="button" class="btn btn-xs btn-danger" onclick="actionDelete(\'' . $row[0] . '\')"><i class="fa fa-trash"></i>  Delete</button>';
							}
							echo '
						</td>
						</tr>';
			...
			<div class="modal-body">
				<form role="form" id="add_cust" action="" method="POST" enctype="multipart/form-data" autocomplete="off">
					...
					<div class="form-group">
						...
						<div class="controls input-append date form_date_a" data-date-format="yyyy-mm-dd" data-link-field="dtp_input1">
...
<div class="modal fade" id="topup_voucher" role="dialog">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<form role="form" action="?pg=admin_adminTopUp" method="POST" autocomplete="off">
				<div class="modal-body">
					<div class="form-group">
						<label>Voucher ID</label>
						<input type="text" class="form-control" id="id-voucher" name="id_voucher" readonly>
					</div>
					<div class="form-group">
						<label>Invoice Number</label>
						<input type="text" class="form-control" name="invoice_num" required="" placeholder="xxx-xxx-xxx">
					</div>
					<div class="form-group">
						<label>Invoice Date</label>
						<div class="controls input-append date form_date_a" data-date-format="yyyy-mm-dd" data-link-field="dtp_input1">
					...
					<div class="form-group">
						<label>Top-up</label>
						<input type="number" class="form-control" name="top-up" required="" placeholder="00">
...
<script>
	function actionTopUp(id) {
		$('#topup_voucher').modal();
		document.getElementById('id-voucher').value = id;
	}
