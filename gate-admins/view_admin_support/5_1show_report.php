...
//cutomer exam
		case 'exam':
			...
			foreach ($exam as $key => $value) {
				...
				if ($id_program == 'P0006' or $id_program == 'P0007' or $id_program == 'P0008' or $id_program == 'P0009' or $id_program == 'P0010' or $id_program == 'P0011' ) {
					$header[4] = 'Microsoft Word 2016';
					$header[5] = 'Microsoft Excel 2016';
					$header[6] = 'Microsoft Power Point 2016';
					$i = 7;
			...
			echo "
				<div style='text-align:center;'>
					...
					<h5> Periode  ".tanggal_indo($start)." sampai ".tanggal_indo($end)." </h5>
				</div>
				<table id=\"tbl\">
					<tr>
						";
        ...
        $n_gagal=1;
				while ($rows = mysqli_fetch_array($dataVoucher)) {
					...
					$dataSchedule = dataSchedule($value);
					$tanggal_indo = tanggal_indo($dataSchedule[0]);
					$result 			= admRpt_Exam($start,$end,$rows[1]);
					$n 						= subjectls($id_program);
					$dataCustomer = dataCustomer($exIdGroup[0]);
						if (mysqli_num_rows($result)!=0) {
							$sesi ++;
...
*/
							...
							// $field[$value][$no] = array(0=>' '.$row[2],1=>$row[1]);
							// 	$nNilai = setNilai($row[3],$row[6]);
							// 	$iN = 2;
							// 	while ($rNilai = mysqli_fetch_array($nNilai)) { //looping save to array for export
							// 		$val = ($rNilai[0] / $rNilai[1]) * 100;
							// 		$nilaiSubject = number_format((float)$val, 2, '.', '') ;
							// 		$field[$value][$no][$iN] = $nilaiSubject;
							// 		$iN++;
							// 	}
							// 	$field[$value][$no][$iN] = number_format((float)$row[4], 2, '.', '') ;
							// 		$isi_1 .= "<tr><td>$no</td>";
							// 	foreach ($field[$value][$no] as $key_nilai => $value_nilai) {
							// 		$isi_1 .="<td>$value_nilai</td>";	
							// 	}
							// 		$isi_1 .= "<td>".$status."</td></tr>";
							// 		$field[$value][$no][$iN+1] = $status2;
							// 		$no++;
						}
				}
				$i = 4;
        ...
        for ($i=1; $i <= $n_fix ; $i++) {
          ...
          $field[$value.'.1'][$i] = array(0=>'',1=>' '.$nim[$i],2=>$nama[$i],3=>number_format((float)$nilai[$i], 2, '.', ''));
          $isi_1 .= "<tr><td>$i</td>";
					$isi_1 .= "<td></td><td>".$nim[$i]."</td><td>".$nama[$i]."</td><td>".number_format((float)$nilai[$i], 2, '.', '')."%</td>";	
					$isi_1 .= "</tr>";
        }
			...
			echo "
				<div style='text-align:center;'>
					<h3>  10% Nilai Tertinggi ".$dataCustomer[1]."</h3>
					<h5> Periode  ".tanggal_indo($start)." sampai ".tanggal_indo($end)." </h5>
				</div>
				<table id=\"tbl\">
					<tr>
