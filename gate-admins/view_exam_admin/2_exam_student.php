          ...
          <div class="tab-pane fade in active" id="tab_1">
            ...
            if ($_SESSION['valin'] == "All students") {
              foreach (showAllStudents($id_g) as $r) {
                $id = $r[0];
                if (studentOnlineCek($id)) {
                 $release = '<button class="btn btn-xs btn-warning" onclick="release(\'' . $id . '\')"><i class="fa fa-sign-out" aria-hidden="true" ></i> Release</button>';
                }else{
                  $release = '<button class="btn btn-xs btn-warning" disabled><i class="fa fa-sign-out" aria-hidden="true" ></i> Release</button>';
                }
                $n = viewEquExm($id);
                $table .= '<tr>
                <td>' . $no . '</td>
              ...
              foreach (showStudentUnexap($id_g) as $r) {
                $id = $r[0];
                if (studentOnlineCek($id)) {
                 $release = '<button class="btn btn-xs btn-warning" onclick="release(\'' . $id . '\')"><i class="fa fa-sign-out" aria-hidden="true" ></i> Release</button>';
                }else{
