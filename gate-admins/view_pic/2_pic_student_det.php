	...
	<div class="col-md-12">
		<div class="panel panel-default">
			<div class="panel-heading">
				<a href="?pg=pic_student" class="btn btn-xs btn-danger"><i class="fa fa-arrow-left"></i> Back</a> <a
					href="?pg=pic_student_edit&ids=<?= $ids ?>" class="btn btn-xs btn-primary"><i class="fa fa-pencil-square-o"></i>
					Edit</a> Student Detail
			</div>
				...
				<div class="row">
					<div class="col-md-12">
						...
						while ($row = mysqli_fetch_array($dataProgram)) {
							...
							echo '<h3>' . $row[2] . ' </h3><br>';
							$table =
							'<table class="table table-xs table-bordered" id="tbStudents' . $n . '"><thead><tr>
							<th scope = "col" > No</th>
							...
							foreach (recordExamStu($ids, $row[5]) as $r) {
								$date = $r[2] . ' ' . $r[3];
								if ($r[2] == null) {
									$cek_date = CovidCekSchedule($r[7]);
									$date = $cek_date[0] . ' ' . $cek_date[1];
								}
								if ($notif != $r[6]) {
									$table .= "
									<tr>
								...
								if ($r[2] != null) {
									$table .= '<td>' . $date . '</td>
									<td><button type="button"  data-id="' . $r[0] . '" data-group="' . $r[4] . '" data-toggle="modal" data-target="#result" class="btn btn-xs btn-info"><i class="fa fa-search-plus"></i> View</button>';
								} elseif ($cek_date[0] != null) {
									$table .= '<td>' . $date . '</td>
									<td><button type="button"  data-id="' . $r[0] . '" data-group="' . $r[4] . '" data-toggle="modal" data-target="#result" class="btn btn-xs btn-info"><i class="fa fa-search-plus"></i> View</button>';
								} else {
									$table .= '<td colspan="2" align="center"> Sistem Lama';
								}
								$table .= '</td>
								</tr>';
								++$no;
