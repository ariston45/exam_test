              ...
              <table class="table table-xs" id="tbExam">
                ...
                while($row = mysqli_fetch_array($result)){
                  if ($row[5]=='pause') {
                    ...
                    $token="<button type=\"button\" class=\"btn btn-xs btn-block btn-warning \" onclick=\"show_token('$row[4]')\" >$row[4]</button>";
                      ...
                      if (cekTimeSchedule($row[0])) {
                        $token="<a href=\"?pg=pic_exam_schedule&CC=".$row[0]."\"><button type=\"button\" class=\"btn btn-xs btn-danger btn-block \"><i class=\"fa fa-key\"></i>  Generate</button></a>";
                  ...
                  <tr>
                    ...
                    }else if ($row[5]=='init'AND (($expTime>=$today[1] AND $row[1]==$today[0]) OR ($row[1]>$today[0]))) {
