...
function sumPar1($id, $idg)
{
  $sql = "SELECT COUNT(b.id_student) FROM exam_group a INNER JOIN exam_session b ON a.exam_code = b.exam_code
	WHERE SUBSTRING_INDEX(SUBSTRING_INDEX(a.group_name,'.',2),'.',-1) = '$id' AND SUBSTRING_INDEX(a.group_name,'.','1') = '$idg'";
  $res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
...
function progReport1($id, $a, $b, $idg)
{
  $id_prog = $id;
  $data_a = $a;
  $data_b = $b;
  ...
  if ($id_prog != 1) {
    $sql = "SELECT a.exam_code,c.program_name,c.sum_question,c.pass_grade,c.id_program
		...
		WHERE b.`status` != 'init' AND SUBSTRING_INDEX(a.group_name,'.','1') = '$idg' AND c.id_program='$id_prog' AND b.date BETWEEN '$data_a' AND '$data_b' GROUP BY a.exam_code";
  } else {
    $sql = "SELECT a.exam_code,c.program_name,c.sum_question,c.pass_grade,c.id_program
		FROM exam_group a INNER JOIN exam_schedule b ON a.exam_code=b.exam_group LEFT JOIN programs c ON SUBSTRING_INDEX(SUBSTRING_INDEX(a.group_name,'.',2),'.',-1)=c.id_program
		WHERE b.`status` != 'init' AND SUBSTRING_INDEX(a.group_name,'.','1') = '$idg' AND b.date BETWEEN '$data_a' AND '$data_b' GROUP BY a.exam_code";
  }
  $res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
  return $res;
}
