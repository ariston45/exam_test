...
function setProgram($idg, $date1, $date2, $programs)
{
  ...
  if ($program != 1) {
    $sql = "SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(a.group_name,'.','2'),'.','-1') id_program
     ...
     b.date BETWEEN '$date_1' AND '$date_2' AND SUBSTRING_INDEX(a.group_name,'.','1') = '$idg' GROUP BY SUBSTRING_INDEX(SUBSTRING_INDEX(a.group_name,'.','2'),'.','-1')";
  } else {
    $sql = "SELECT SUBSTRING_INDEX(SUBSTRING_INDEX(a.group_name,'.','2'),'.','-1') id_program
     FROM exam_group a INNER JOIN covid_exam_schedule b ON a.exam_code = b.exam_group INNER JOIN
     (SELECT aa.exam_code FROM exam_session aa GROUP BY aa.exam_code) c ON a.exam_code=c.exam_code
...
function setGroup($id, $a, $b, $idg)
{
  $sql = "SELECT a.exam_code,a.id_student,b.classroom,b.proctor,b.date
  FROM exam_session a INNER JOIN covid_exam_schedule b ON a.exam_code = b.exam_group
  INNER JOIN exam_group c ON a.exam_code=c.exam_code WHERE SUBSTRING_INDEX(SUBSTRING_INDEX(c.group_name,'.',2),'.',-1) = '$id'
  AND b.date BETWEEN '$a' AND '$b' AND SUBSTRING_INDEX(c.group_name,'.',1) = '$idg'";
  $res = mysqli_query($GLOBALS['link'], $sql) or die(mysqli_error($GLOBALS['link']) . '<br>' . $sql);
  return $res;
}
...
// $nop = setGroup($id_program, $date1, $date2, $idg);
// echo $nop;
//////////////////////////////////////////////////////////////
// die();
$arr = array();
...
if ($acc == 0) {
  ...
  if ($view[2] != "0") {
    while ($dat = mysqli_fetch_array($prog)) { #looping program dalam sheet
      ...
      $np = substr($nameprogram, 0, 25);
      $spreadsheet->createSheet($num)->setTitle($np); //looping program
      $a = 2;
      $r = 1;
      ...
      $data_1 = setGroup($id_program, $date1, $date2, $idg);
      $no = 1;
      $a++;
      while ($row = mysqli_fetch_array($data_1)) {
        ...
        $spreadsheet->setActiveSheetIndex($num)->setCellValue('F' . $a, $id_str)->getColumnDimension('F')->setAutoSize(true);
        ...
        while ($row = mysqli_fetch_array($nilai)) {
          ...
          $arr[$idx] = $par;
          $spreadsheet->setActiveSheetIndex($num)->setCellValue($par . $a, $nilaiSubject)->getColumnDimension($par)->setAutoSize(true);
          $col_1[] = $par;
          $par++;
          $idx++;
        }
