...
<div class="row">
  ...
  <div class="col-md-12">
    <div class="panel panel-primary">
      <div class="panel-heading">Lab Exam Schedules</div>
  ...
  if ($rs_config[1]==1) { ?>
	  <div class="panel-heading">Home Exam Schedules</div>
        <div class="panel-body">
          <!-- <h4>Exam</h4> -->
            <table class="table table-xs" id="tbhomeExam">
              <thead>
                <tr>
                  <th scope="col">No.</th>
                  <th scope="col">Date</th>
              ...
              <tbody><?php
              $no = 1;
              $result = CovidshowExamDashProc($_SESSION['admin_group']);
              while ($row = mysqli_fetch_array($result)) {
                  if ($row[4] != null) {
                    if ($row[1] >= $today[0]) {
                       $token = "<button type=\"button\" class=\"btn btn-xs btn-block btn-warning \" onclick=\"show_token('$row[4]')\" >$row[4]</button>";
                      ...
                      if ($row[1] >= $today[0]) {
                          $token = '<a href="?pg=home_proc_exam&CC='.$row[0].'"><button type="button" class="btn btn-xs btn-danger btn-block "><i class="fa fa-key"></i>  Generate</button></a>';
                      } else {
                          if ($row[1] == $today[0] and $row[2] > $today[1]) {
                              $token = '<button type="button" class="btn btn-xs btn-default btn-block ">Not Ready</button>';
                          } elseif ($row[1] > $today[0]) {
                ...
                <tr>
                  ...
                  <td>';
                  if ($row[5] != 'init') {
                      echo '<a href="?pg=home_proc_sch_detail_a&CC='.$row[0].'"><button type="button" class="btn btn-xs btn-primary"><i class="fa fa-info-circle"></i>  Detail</button></a> ';
                  }
                  echo '</td></tr>';
                  ++$no;
              }?>
